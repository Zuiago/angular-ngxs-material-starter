    ...
    return (c: FormControl) => {
        if (c.value && c.value.toString() !== '') {
            ...
            if (stringMask && stringMask.pattern.length > 8) {
                const maskLength = stringMask.pattern.trim().replace(/[^0-9]/g, '').slice(0, 13).length;
...
export class PhoneDirective implements OnInit, ControlValueAccessor {
    ...
    public onChangeCallback = (_: any) => {/*Vazio*/
    };
    @HostListener('blur', ['$event'])
    public onTouchCallback = () => {/*Vazio*/
    };
    validateFn: any = () => {/*Vazio*/
    };
    ...
    public _cleanValue(viewValue): string {
        if (this.phonePattern) {
            const maskLength = this.phonePattern.pattern.trim().replace(/[^0-9]/g, '').slice(0, 13).length;
